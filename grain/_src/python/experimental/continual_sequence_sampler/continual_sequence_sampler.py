...
def shard_elements(
  ...
  return (
      [element_length for _, element_length in new_clip_map],
      [i for i, _ in new_clip_map],
  )


def get_shuffled_element_index(
...
def _element_clip_from_index(
  ...
  if current_element_start <= index_within_epoch < next_element_start:
    ...
    element_id = get_shuffled_element_index(
...
class ContinualSequenceSampler:
  ...
  def _compute_start_index_tree(self, epoch: int) -> np.ndarray:
    ...
    shuffled_indices = [
        get_shuffled_element_index(i, epoch, self._element_index)
  ...
  def current_element_index(self) -> int:
    ...
    return get_shuffled_element_index(
...
class BatchedContinualSequenceSampler:
  ...
  def __init__(
    ...
    assert (
        self._batch_size % shard_options.shard_count == 0
...
class SamplerWrapper:
  """Wraps a ContinualSequenceSampler to conform to the interface."""
...
def get_sampler(
  ...
  if "shard_options" in kwargs:
    sampler = BatchedContinualSequenceSampler(clip_map, seed=seed, **kwargs)
  else:
